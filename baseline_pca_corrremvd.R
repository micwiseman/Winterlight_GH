...
# Ensure you're selecting only numeric columns
numeric_cols <- sapply(Jou_FA_MDD, is.numeric)

# Calculate the variance for these columns
numeric_cols_variance <- sapply(Jou_FA_MDD[, numeric_cols], function(x) var(x, na.rm = TRUE))
# Identify columns with zero variance
cols_with_zero_variance <- names(numeric_cols_variance[numeric_cols_variance == 0])
# Remove columns with zero variance
Jou_FA_MDD <- Jou_FA_MDD[, !(names(Jou_FA_MDD) %in% cols_with_zero_variance)]
...
Jou_FA_MDD <- Jou_FA_MDD[, !(names(Jou_FA_MDD) %in% columns_to_remove)]
# Scale the data
