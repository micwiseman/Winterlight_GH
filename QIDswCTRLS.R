...
QIDS <- read_csv("~/Lab/Winterlight/PsychiatryData_for_RabinLab.csv")
col_index<-which(colnames(QIDS)=="qids_total_pre")
colnames(QIDS)[col_index] <-"Baseline_QIDS"
colnames(QIDS)[1] <- "participant_external_id"
WL_Jou_BLq <- merge(WL_Jou_BL, QIDS[,c("participant_external_id", "Baseline_QIDS")], by = "participant_external_id", all.x = TRUE)
...
# Separate by subtask
WL_feeling_BLq <- subsetStimulus(WL_Jou_BL, "en_instruction_journal_feeling.mp3")
...
for (outcome_var in outcome_variables) {
  # Create linear model
  lm_formula <- as.formula(paste(outcome_var, "~ Baseline_QIDS + sex + age_screening + testing_location + age_learned_english"))
  lm <- lm(lm_formula, data = WL_feeling_BLq)
  
  # Store results in the database
  result_row <- data.frame(
    outcome_variable = outcome_var,
    beta_coefficient = formatC(summary(lm)$coefficients[2, 1], format = "f", digits = 3),
    test_statistic = formatC(coef(summary(lm))[2, 3], format = "f", digits = 3),
    standard_error = formatC(coef(summary(lm))[2, 2], format = "f", digits = 3),
    p_value = formatC(coef(summary(lm))[2, 4], format = "f", digits = 3),
    stringsAsFactors = FALSE
  )
  results <- rbind(results, result_row)
  # Plot scatter plot of the data
  plot(WL_feeling_BLq[[outcome_var]] ~ WL_feeling_BLq$Baseline_QIDS, 
       xlab = "Baseline_QIDS", ylab = outcome_var,
       main = paste("Scatter Plot - Outcome:", outcome_var))
  # Add regression line using the second coefficient
  abline(a = coef(lm)[1], b = coef(lm)[2], col = "red")
}

# Print the results
print(results)

linear_regression_assumptions(WL_feeling_BLq, "short_pause_count_normalized", c("Baseline_QIDS", "sex", "age_screening", "testing_location","age_learned_english"))
model <- lm(fundamental_frequency_variance ~ Baseline_QIDS + sex + age_screening + testing_location+ age_learned_english, data = WL_feeling_BLq)
scatter <- ggplot(WL_feeling_BLq, aes(y = "fundamental_frequency_variance", x = Baseline_QIDS)) +
