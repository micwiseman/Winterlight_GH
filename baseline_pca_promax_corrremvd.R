...
# Print the max number of missing values after imputation and the dimensions of the imputed data
missing_values_after_imputation
dim(missing_values_after_imputation)
...
cor_matrix <- cor(Jou_FA_MDD[sapply(Jou_FA_MDD, is.numeric)], use="pairwise.complete.obs")
# Define a threshold for maximum allowed correlation
cor_threshold <- 0.9
# Create a data frame of the correlation pairs above the threshold
...
# Create a data frame of the pairs with their correlation
high_cor_df <- data.frame(row = rownames(cor_matrix)[cor_pairs[,1]],
                          column = rownames(cor_matrix)[cor_pairs[,2]],
                          cor_value = cor_matrix[cor_pairs])
# Order by absolute correlation value
high_cor_df <- high_cor_df[order(-abs(high_cor_df$cor_value)),]

# Now you can decide which variables to remove based on the ordered correlations
# For instance, you can choose to always remove the second variable in each pair
columns_to_remove <- unique(high_cor_df$column)
...
# Impute the remaining missing values with the mean of each column
# Note: This only works for numerical columns
numerical_columns <- sapply(data_scaled, is.numeric)
data_scaled[numerical_columns] <- lapply(data_scaled[numerical_columns], function(x) {
  ifelse(is.na(x), mean(x, na.rm = TRUE), x)
})

# Verify that there are no more missing values
missing_values_after_imputation <- sum(is.na(data_scaled))
colnames(data_scaled)[colMeans(is.na(data_scaled)) >= 0.7]
# Take out features where the most values are NA 
data_scaled <- data_scaled[, colMeans(is.na(data_scaled)) < 0.7]
# Find and print the column names with all values equal to 0 or NA
zero_or_na_cols <- colnames(data_scaled)[apply(data_scaled, 2, function(col) all(is.na(col) | col == 0))]
cat("Columns with all values equal to 0 or NA:", zero_or_na_cols, "\n")
data_scaled <-  data_scaled[!apply(data_scaled, 2, function(col) all(is.na(col) | col == 0))]
# Select numeric columns
numeric_cols <- names(data_scaled)[sapply(data_scaled, is.numeric)]
# Calculate the variance for numeric columns
variance <- apply(data_scaled[, numeric_cols, drop = FALSE], 2, function(x) var(x, na.rm = TRUE))
# Filter columns with variance less than or equal to 0.001
selected_columns <- names(variance[variance == 0])
data_scaled <- data_scaled[,!names(data_scaled) %in% selected_columns]


library(psych)
library(GPArotation)
# Perform PCA
fa_result <- fa(data_scaled, nfactors = 10, rotate = "promax")
...
# Extract the rotated loadings for further analysis
loadings <- fa_result$loadings# Scree Plot Look for the point where the plot starts to flatten out (the 'elbow'), 
...
abs_loadings <- abs(fa_result$loadings)  # Using absolute values of loadings
...
for (cat_name in names(columns_by_category)) {
  for (pc in 1:num_pcs) {
    ...
    common_vars <-intersect(cat_vars, rownames(significant_loadings))
    cat_loadings <- significant_loadings[common_vars, pc]
    
    # Calculate the proportion of significant variables
    num_significant <- sum(cat_loadings, na.rm = TRUE)
    total_vars_in_category <- length(common_vars)
