...
WL_Jou<- merge(WL_Jou, demoMDD, by = "participant_external_id", all.x=TRUE)
MDD_Jou <- WL_Jou %>% filter(Group == "MDD")
...
col_index2<-which(colnames(HAMD)=="hamd17_total_wk2")
colnames(HAMD)[col_index2] <-"Week2_HAMD"
col_index4<-which(colnames(HAMD)=="hamd17_total_wk4")
colnames(HAMD)[col_index4] <-"Week4_HAMD"
colnames(HAMD)[1] <- "participant_external_id"
MDD_Jou <- merge(MDD_Jou, HAMD[,c("participant_external_id", "Baseline_HAMD", "Week2_HAMD", "Week4_HAMD")], by = "participant_external_id", all.x = TRUE)


# Find participants who don't have all HAMD data 
missing_baseline <- MDD_Jou$participant_external_id[is.na(MDD_Jou$Baseline_HAMD)]
missing_week2 <- MDD_Jou$participant_external_id[is.na(MDD_Jou$Week2_HAMD)]
missing_week4 <- MDD_Jou$participant_external_id[is.na(MDD_Jou$Week4_HAMD)]



# Separate by subtask
WL_feeling <- subsetStimulus(MDD_Jou, "en_instruction_journal_feeling.mp3")
# Take out duplicates 
...
WL_feeling$percent_reduction <- (WL_feeling$Baseline_HAMD-WL_feeling$Week4_HAMD) / WL_feeling$Baseline_HAMD * 100

# Classify responder status for MDD participants
WL_feeling$Group <- ifelse(
  WL_feeling$Group == "MDD" & WL_feeling$percent_reduction >= 50,
  "Responder",
  ifelse(
    WL_feeling$Group == "MDD" & WL_feeling$percent_reduction < 50,
    "Non-responder",
...
num_unique_responders <- length(unique(WL_feeling$participant_external_id[WL_feeling$Group == "Responder"]))
num_unique_non_responders <- length(unique(WL_feeling$participant_external_id[WL_feeling$Group == "Non-responder"]))
# Print the counts
...
visit_summary <- WL_feeling %>%
  group_by(session_label, Group) %>%
  summarize(
